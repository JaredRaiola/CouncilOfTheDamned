...
// reviewers rate proofs, so they see them, but capped: head 150 + tail 250 keeps the command echo and the result line at the end of a pasted run
...
const planRule = planned ? `The brief carries a plan from the design stage. Execute it task by task: read that task's files once, make its edits, run its check, move on. Do not re-derive the design or restructure the plan. Deviate only where the plan is wrong against the repo, and record each deviation in \`risks\` as "Ruling: <what you did instead> — <why>".`
  // planFirst: no design stage ran; each builder writes its own plan first, executes it, then self-reviews against it before submitting
  : A.planFirst ? `Plan first, then build, then self-review. (1) Before touching code, write your implementation plan to \`.council-plan.md\` in your worktree: tasks, each with files as "path:startLine-endLine action", one-action steps carrying the actual code, the command to run and its expected output, and the brief workflows it proves; no placeholders. (2) Execute it task by task: read that task's files once, make its edits, run its check, move on. (3) When the suite is green, re-read your whole diff against the plan and the brief as a reviewer would: missed requirements, placeholders, names that drifted, proofs that are not output. Fix what you find. (4) Delete \`.council-plan.md\` before your final commit; put the task list in \`summary\`.` : ''
const trim = (s) => ({ ...s, testOutput: String(s.testOutput ?? '').slice(-2000), workflows: s.workflows.map(w => ({ ...w, examples: w.examples.map(e => ({ ...e, proof: cap(e.proof), observed: cap(e.observed) })) })) })
...
You are council member ${LABELS[i]}. Implement the task below. ${planRule}
...
2. For each affected workflow, provide at least ${floor.perWorkflow} DISTINCT proven examples: different inputs, edge values, error paths. Proven = either (a) a test case in your testCommand run, with \`proof\` = that case's result line from your FINAL run, or (b) a driven run whose output you paste in \`proof\`; never run examples one at a time. Described-but-not-run does not count.
